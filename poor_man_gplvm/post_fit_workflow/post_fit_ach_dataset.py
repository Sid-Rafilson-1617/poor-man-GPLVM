    ...
    for feat_name,feat in feature_d.items():
        ...
        for interval_name,interval in interval_d.items():
            if isinstance(interval,nap.IntervalSet):
               ...
               mean_feature_d[feat_name,interval_name] = feat.interpolate(interval)
...
def feature_distance_vs_label_distance_analysis(prep_res,label_intv,ach_intv=None,ach_onset=None,ach_extend_win=1,feature_key_l=['p_latent','pv'],interval_key_l=['ACh_onset','ripple'],n_shuffles=200,label_distance_threshold=None,mask_d={}):
    ...
    if 'ripple' in interval_key_l:
        if 'is_ripple' in prep_res:
            ...
            interval_d['ripple'] = interval_d['ripple'][interval_d['ripple'].d] # indices
        else:
...
def gather_feature_shuffle_across_sessions(analysis_res_d_allsess,prep_fig_save_dir='./',ylim_d={}):
    ...
    #     ax.plot(one_shuffle_allsess.T- to_shift.values[None,:],alpha=0.3,linestyle=':')
        ...
        fig_d[feature_key,event_key] = fig
...
def latent_cluster_vs_timing_regression(cluster_label_l,event_ts,nrem_intv,do_print=True,n_shuffle=100,quantile=0.99,alpha=0.01,regularization_method='l1'):
    ...
    if regularization_method is None:
        ...
        reg_res = model.fit_regularized(method=regularization_method,alpha=alpha)
    if do_print:
